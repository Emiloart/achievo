...
let StorageService = class StorageService {
    constructor() {
        const driver = (process.env.PROOF_STORAGE_DRIVER || "LOCAL").toUpperCase();
        this.driver = driver;
        if (driver === "S3") {
            this.adapter = new S3StorageAdapter();
            return;
        }
        const baseDir = process.env.PROOF_LOCAL_DIR || (0, path_1.join)(process.cwd(), "storage", "proofs");
        this.adapter = new LocalDiskStorageAdapter(baseDir);
    }
    getDriver() {
        return this.driver;
    }
...
export const StorageService = exports.StorageService as any;
